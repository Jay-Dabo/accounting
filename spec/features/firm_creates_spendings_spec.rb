...
feature "FirmCreatesSpendings", :spending do
  ...
  describe "Firm adds new spending record" do
    ...
    let!(:cash_balance) { balance_sheet.cash_sens + capital.amount }
  	...
  	describe "purchasing other current asset" do
  		...
  		describe "check changes in balance sheet" do
  			...
  			it { should have_css('th#cash', text: 0) } # for the cash balance
  			it { should have_css('th#other_current', text: 10500500) } # for the other curr asset balance
  	...
  	describe "purchasing plant" do
        ...
        before { click_neraca(2015) }
  			...
  			it { should have_css('th#fixed', text: balance_sheet.fixed_assets_sens + 10500500) } # for the fixed asset balance
        it { should have_css('div.debug-balance' , text: 'Balanced') }
  		end
  	end
  	...
  	describe "firm adds marketing expense" do
        ...
        it { should have_css('th#retained', text: income_statement.retained_earning_sens - 5500500) } # for the retained earning balance
  		end

      describe "check changes in balance sheet" do
        before { click_neraca(2015) }
        it { should have_css('th#cash', text: cash_balance - 5500500) } # for the cash balance
        it { should have_css('th#retained', text: balance_sheet.retained_sens - 5500500) } # for the retained balance
        it { should have_css('div.debug-balance' , text: 'Balanced') }
      end
  	end
  end
