...
feature "FirmSellsAsset", :type => :feature do
  subject { page }
  ...
  describe "selling asset" do
  	before do
        ...
        select asset_1.asset_name, from: 'revenue_revenue_item'
        fill_in("revenue[quantity]", with: asset_1.unit, match: :prefer_exact)
        fill_in("revenue[measurement]", with: asset_1.measurement, match: :prefer_exact)
	  ...
	  describe "check changes in asset table" do
	  	before do
	  		visit user_root_path
	  		click_link "Daftar Lengkap Aset Milik Usaha"
	  	end
