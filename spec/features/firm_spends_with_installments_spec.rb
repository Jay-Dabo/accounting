  ...
  describe "Firm spends with installment" do
	describe "when purchasing fixed asset" do
		before do
			...
			select 'Mesin, Fasilitas Produksi', from: 'spending_item_type'
			fill_in("spending[item_name]", with: "Bengkel di Kemang")
			fill_in("spending[total_spent]", with: total_spent)
			fill_in("spending[quantity]", with: 1)
			fill_in("spending[measurement]", with: "potong")
			...
			it { should have_content('Pengeluaran berhasil dikoreksi') }
	  		...
	  		describe "check changes in balance sheet" do
				...
				# it { should have_content('Pengeluaran berhasil dikoreksi') }
	  			...
	  			it { should have_css('#payables', text: balance_sheet.payables + total_spent + 1000000 - dp_paid) } # for the payables balance
	  			it { should have_css('div.debug-balance' , text: 'Balanced') }
	  		end
  		end
	end	
 ...
 #  		end
	...
	describe "when paying expense" do
		before do
			...
			# fill_in("spending[year]", with: 2015)
			fill_in("spending[info]", with: "Hasil Negosiasi")
			select 'Pemasaran', from: 'spending_item_type'
			fill_in("spending[item_name]", with: "Bengkel di Kemang")
			fill_in("spending[total_spent]", with: total_spent)
			fill_in("spending[quantity]", with: 1)
			fill_in("spending[measurement]", with: "potong")
